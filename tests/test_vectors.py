...
def test_malicious_nopasswd(auditor):
    ...
    assert any("WARNING: 'NOPASSWD' tag used" in issue for finding in result.findings for issue in finding.issues)

def test_malicious_wildcard(auditor):
    result = auditor.audit_file(get_vector_path('malicious_wildcard.sudoers'))
    assert result.error is None
    assert len(result.findings) > 0
    assert any("HIGH: Wildcard '*' detected" in issue for finding in result.findings for issue in finding.issues)

def test_malicious_all_cmd(auditor):
    result = auditor.audit_file(get_vector_path('malicious_all_cmd.sudoers'))
    assert result.error is None
    assert len(result.findings) > 0
    assert any("CRITICAL: 'ALL' command granted" in issue for finding in result.findings for issue in finding.issues)
...
def test_malicious_gtfobins(auditor):
    ...
    assert any("CRITICAL: GTFOBins detected" in issue for finding in result.findings for issue in finding.issues)
...
def test_malicious_mixed(auditor):
    ...
    assert result.error is None
    assert len(result.findings) >= 4 # We expect at least 4 findings based on the file content
